...
def shrinked_img(
    ...
    h_indexis = np.where(img_array.min(axis=0) < 255)
    v_indexis = np.where(img_array.min(axis=1) < 255)
    h_min, h_max = h_indexis[0].min(), h_indexis[0].max()
    v_min, v_max = v_indexis[0].min(), v_indexis[0].max()
